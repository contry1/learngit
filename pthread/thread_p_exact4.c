 ...
 int main() {
    ...
    // sf =  realloc(sf,sizeof(struct line));
     ...
     int i = 0;
     create_thread(&thread_id,(void *)sf,main_func);
     sleep(1);
     create_thread(&thread_id2,(void *)sf,second_func);

     sleep(3);
     int err = 0;
     wait_pthread(thread_id);
     wait_pthread(thread_id2);
 ...
 void wait_pthread(pthread_t pid )
 {
     int errno = pthread_join(pid,NULL);
     if (!errno) {
     /* code */
     printf("wait success thread :%ld\n",pthread_self());
     }else
