typedef struct Observer_t {
	void(*Update_pull)(struct Observer_t *,void *this1);
}Observer;
...
void NotifyObservers1(struct concrede_subject * this1)
{
	...
	for (i = 0; i < this1->obs_cnt; ++i)
	{
		if (NULL == this1->obs[i]->Update_pull)
		{
		}
		else
		{
			//这里我们调用的是observer中的update方法，如果具体类中display方法放在前面，为何在具体类中，调用的却是display1方法！
			this1->obs[i]->Update_pull(this1->obs[i],(void*)this1);
		}
...
typedef struct concrede_display {
	void(*Update_pull)(struct concrede_display *this2,WeatherData * we);//必须放在结构体第一位！
...
void Update11_pull(struct concrede_display *this2, WeatherData * we);//必须放在结构体第一位！
...
void Update11_pull(struct concrede_display *this2, WeatherData * we)
{
	if (!we)
		return;
	this2->temp = we->temp;
	this2->humidity = we->humidity;//这里可以通过get方法去获取到WeatherData 数据，如果WeatherData已经开放了get 接口
	Display11(this2);
