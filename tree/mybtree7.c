...
/* the belw is exactly insert func */
bitree  * myinsertbt(bitree  *node,int n)
{
...
/* 输的广度优先遍历  利用栈的性质,有bug,不能用栈表示*/
void  wideorder(bitree * bt)
{
	bitree  * stack[20] ={0};
	bitree  * root =NULL;
	bitree  * isnode =NULL;
...
void levelorder(bitree  *bt)
{
	...
	if(root)
	{
		...
		while(end!=0&&root!=NULL&&end!=first)
		{
			node = queue[first];
			if(node->left)
			{
...
void printTreeAtLevel(bitree * tree)
{
	...
	do
	{
		...
		{
			queue[end] = queue_node->right ; 
			end++ ; 
		}

		printf("%d\t" ,queue_node->key) ; 
		first++ ; 
...
/* 深度优先遍历 */
/* 思想： 1.现将根节点入站，出战，top》-1
		  2.如果当前节点的有孩子存在，入站，否则3*
		  3.如果做孩子节点存在，入站，
		  4.循环1，2，3，直到top《=-1*/
...
int main()
{
	...
	//_midorder(t);
	printf("\n");
	//_lastorder(t);//the _lastorder keyi he preorder  midorder同时使用。
