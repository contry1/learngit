   ...
   */
   ...
   void connect_t(int fd,const struct sockaddr *addr, socklen_t addrlen);
   void connect_attr_t(int flag,int fd,const struct sockaddr *addr, socklen_t addrlen);
   void fd_noblock(int fd);
   ...
   int main() {
      ...
      if (create_and_set_socket(&sockfd,&opt_r)==NULL) 
       ...
       // connect_t(sockfd,(const struct sockaddr *)&server, sizeof(server));
    connect_attr_t(O_NONBLOCK,sockfd,(const struct sockaddr *)&server, sizeof(server));
       ...
       void sigio(int signo)
       {
           printf("the server read close,and the client to write,so make sigpipe \n");
           close(sockfd);
