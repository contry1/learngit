...
int main() {
  ...
  while(1) {
    ...
    if (nread == -1)
      perror("nread");
    //write(STDOUT_FILENO,buf,nread); =fwrite(buf,nread,1,stdout);=printf("the buf recv:%s\n",buf );when setbuf(stdout,null);

    setbuf(stdout,buf);//there is a bug .that is the client 1th is not write to stdout
    fflush(stdout);
    fwrite(buf,nread,1,stdout);

    // reinit the buf,as of the buf is start the last point end,not 0 
    memset(buf,'\0',MAXNITEMS);
