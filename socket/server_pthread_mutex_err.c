...
socklen_t  addrlen;   
// void *query(void * arg);
void gethostbyaddr_t( struct hostent *gethost);
...
int main() {
 ...
 // /init the lock
  ...
  if(bind(sf->listenfd, (struct sockaddr *)&server, sizeof(server)) ==   -1) {
   ...
   //this is example to call gethostbyaddr
   struct hostent *gethost = NULL;
   gethostbyaddr_t(gethost);
...
void gethostbyaddr_t( struct hostent *gethost)
{
       ...
       gethost = gethostbyaddr(&ipv4addr, sizeof(ipv4addr), AF_INET);
       if (gethost == NULL) {/* code */
            perror("gethost");
       }else {
           printf("host name:%s\n",gethost->h_name );
