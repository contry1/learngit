        ...
        if(FD_ISSET(sf->listenfd, &rset)){
           ...
           for (i = 0; i < BACKLOG; i++) 
            ...
            if (sf->connectfd > maxfd)
                maxfd = sf->connectfd; can comment

            }
            
            // /deal to connected client  data transfer
            for (int i = 0; i <= BACKLOG; i++)//there we use backlog,it will loop many time than cnt, so we do not use itusually
            {
                
              /*  {sockfd = client[i]; if (sockfd > 0)... } == {if ((sockfd = client[i])<0) continue; }
*/
                sockfd = client[i];
                if (sockfd > 0) {
